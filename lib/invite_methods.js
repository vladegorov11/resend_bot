...
const resultFormater = function( fail, success, invitedUsers) {
	return result = {
		count: `\u{2714} <b> Пользователей добавлено: </b> ${success} \n\u{2716} <b> Пользователей не добавлено: </b> ${fail}`,
		users: invitedUsers
	};
};
...
const inviteToGroup = async(from, to, users, inviteLogProperties) => {
    ...
    try {
      ...
      invitedUsers.push(users[i].first_name + ' ' + users[i].last_name); //output
      success++;
    } catch(e) {
